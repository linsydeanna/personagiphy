...
export const testEpic = action$ =>
  action$.filter(action => action.type === 'ADD_REQUEST') // difference between filter and ofType?
  .mergeMap(() => fetch(`http://api.giphy.com/v1/gifs/search?q=michael+jackson&api_key=${API_KEY}&limit=5`)
    .then(res => res.json())
